...
%% LOAD FLEET DATA W/ FUEL TYPE
%Load fleet data w/ fuel types (for getting emissions by coal)
genfueltypedir = 'C:\Users\mtcraig\Desktop\EPP Research\Matlab\Fleets';
genfueltypefile = 'CPPEEPt5FlxCCS1MW4000WndMW0Rmp1MSL1Vnt0LwrLmtNRELResSSH124LA.csv';
if size(genfueltypefile,2)>31
    sheetname=genfueltypefile(1:31);
else
    sheetname=genfueltypefile;
end
[~,~,genfueltypedata]=xlsread(fullfile(genfueltypedir,genfueltypefile),sheetname);
%Get column numbers for fuel type data
oriscolfueltypesheet=find(strcmp(genfueltypedata(1,:),'ORISCode'));
genidcolfueltypesheet=find(strcmp(genfueltypedata(1,:),'UnitID'));
genfueltypecolfueltypesheet=find(strcmp(genfueltypedata(1,:),'FuelType'));
affectedegucolfueltypesheet=find(strcmp(genfueltypedata(1,:),'Affected EGU CPP'));
%Convert ORIS & unit ID cols to strings
for i=2:size(genfueltypedata,1)
    genfueltypedata{i,oriscolfueltypesheet}=num2str(genfueltypedata{i,oriscolfueltypesheet});
    if isnumeric(genfueltypedata{i,genidcolfueltypesheet})
        genfueltypedata{i,genidcolfueltypesheet}=num2str(genfueltypedata{i,genidcolfueltypesheet});
    end
end
...
coalco2emsalldata=find(strcmp(alldata(1,:),'TotalCoalCO2CPPProduction(kg)'));
...
%Also hold CO2 CPP emissions by coal generator
coalccsgenidsfile='CoalGenIDsCCS4GW25July2016.xlsx';
[~,~,coalgensccs]=xlsread(fullfile('C:\Users\mtcraig\Desktop\EPP Research\MatlabScripts',coalccsgenidsfile),'Sheet1');
coalco2cppemsbygen=coalgensccs;
orisunitcolcoalgen=find(strcmp(coalco2cppemsbygen(1,:),'ORIS-UnitID'));
...
for folderctr=1:size(foldernames,1)
    ...
    actualreservecost=0; totalcoalco2ems=0;
    ...
    %Empty set of coal plants included in emissions
    coalplantswithco2cppems={};
    coalco2cppemsbygen{1,size(coalco2cppemsbygen,2)+1}=currfolder;
...
%             reservedatatemp=ExtractDataPLEXOSAnnual(fiscalyeardir,resfilename);
        ...
        %% CALCULATE EMISSIONS BY COAL PLANTS
        %Need to look up fuel type of generator, and if generator is coal,
        %add emissions to coal.         
        %Break out ORIS & gen ID (currunitid is ORIS-UNIT)
        dashlocs=findstr(currunitid,'-');
        currorisid=currunitid(1:dashlocs(1)-1);
        currgenid=currunitid(dashlocs(1)+1:end);
        
        %Some generators that ARE NOT coal plants don't have a gen id.
        %If no gen id, just skip since only interested in coal plants. 
        if not(isempty(currgenid))
            %Get row of curr plant in fuel type data
            unitrow=find(strcmp(genfueltypedata(:,oriscolfueltypesheet),currorisid) &...
                    strcmp(genfueltypedata(:,genidcolfueltypesheet),currgenid));
                
            %Some gen IDs interpreted by Excel as dates - see if can match
            %on that if haven't matched already.
            if isempty(unitrow)
                dashloc = findstr(currgenid,'-');
                currgenidasdate=strcat(currgenid(1:dashloc-1),'/',currgenid(dashloc+1:end),'/2016');
                unitrow=find(strcmp(genfueltypedata(:,oriscolfueltypesheet),currorisid) &...
                        strcmp(genfueltypedata(:,genidcolfueltypesheet),currgenidasdate));
%                 strcat('Convereted to date ',currunitid,'oris',currorisid,'gen',currgenid)
            end
            
            %Finally, some PLEXOS IDs are 01, but fuel type gen IDs are 1
            %(or similar #s). Try to match these.
            if isempty(unitrow)
                currgenid=num2str(str2num(currgenid));
                unitrow=find(strcmp(genfueltypedata(:,oriscolfueltypesheet),currorisid) &...
                        strcmp(genfueltypedata(:,genidcolfueltypesheet),currgenid));
            end
            
            if not(isempty(unitrow))
                currfueltype=genfueltypedata{unitrow,genfueltypecolfueltypesheet};
                %Get whether affected EGU under CPP
                curraffectedegu=genfueltypedata{unitrow,affectedegucolfueltypesheet};
                %Get emissions of generator if affected coal plant & add to total
                if strcmp(currfueltype,'Coal') && curraffectedegu==1
%                     strcat('ORIS ID',num2str(currorisid),'GenID',num2str(currgenid))
                    %Get plant emissions
                    genemsfilename=strcat('ST Emission(',num2str(co2cppid),').Generators(',num2str(currplexosid),').Production');  
                    geneco2cppemsdata = ExtractDataPLEXOSAnnual(fiscalyeardir,genemsfilename);            
                    generatorco2cppproduction = geneco2cppemsdata{2,4}*scaleresults;

                    %Store coal plant list
                    coalplantswithco2cppems = vertcat(coalplantswithco2cppems,strcat(currorisid,{' '},currgenid));

                    %Add to total
                    totalcoalco2ems = totalcoalco2ems + generatorco2cppproduction;
                    
                    %If flex CCS generator, find row in
                    %coalco2cppemsbygentemp and store emissions
                    currgenrowcoalems = find(strcmp(coalco2cppemsbygen(:,orisunitcolcoalgen),currunitid));
                    if not(isempty(currgenrowcoalems))
                        coalco2cppemsbygen{currgenrowcoalems,end}=generatorco2cppproduction;
                    end
                end
            end
        end
...
%     alldata{newrow,reservecostcolalldata}=reservecosttotal;
    ...
    alldata{newrow,coalco2emsalldata}=totalcoalco2ems;
